...
use mfs_i18n_build::artifacts::write_id_map_entries;
use mfs_i18n_build::catalog::Catalog;
use mfs_i18n_build::catalog_reader::{CatalogReadError, load_catalog};
use mfs_i18n_build::compiler::{CompileError, compile_message};
use mfs_i18n_build::id_map::IdMap;
use mfs_i18n_build::locale_sources::{LocaleBundle, LocaleSourceError, load_locales};
use mfs_i18n_build::manifest::{Manifest, PackEntry, sha256_hex};
use mfs_i18n_build::micro_locales::{MicroLocaleError, load_micro_locales};
use mfs_i18n_build::pack_encode::{PackBuildInput, encode_pack};
use mfs_i18n_build::parser::parse_message;
use mfs_i18n_build::platform::{
...
pub fn run_build(options: &BuildOptions) -> Result<(), BuildCommandError> {
    ...
    for locale in locales {
        ...
        let pack_kind = if parent.is_some() {
            mfs_i18n_core::PackKind::Overlay
        } else {
            mfs_i18n_core::PackKind::Base
        };
        ...
        let entry = PackEntry {
            kind: match pack_kind {
                mfs_i18n_core::PackKind::Base => "base".to_string(),
                mfs_i18n_core::PackKind::Overlay => "overlay".to_string(),
                mfs_i18n_core::PackKind::IcuData => "icu_data".to_string(),
            },
    ...
    for (key, id) in &platform_id_map {
        id_map
            .insert(key.clone(), mfs_i18n_core::MessageId::new(*id))
...
fn compile_locale_messages(
    locale: &LocaleBundle,
    catalog: &Catalog,
) -> Result<BTreeMap<mfs_i18n_core::MessageId, mfs_i18n_core::BytecodeProgram>, BuildCommandError> {
    ...
    for message in &catalog.messages {
        ...
        messages.insert(mfs_i18n_core::MessageId::new(message.id), compiled.program);
    }
    Ok(messages)
}
...
mod tests {
    ...
    use mfs_i18n_build::id_map::IdMap;
    use mfs_i18n_build::platform::derive_id_map_entries_from_catalog;
    use std::fs;
    use std::path::PathBuf;
    ...
    fn temp_dir() -> PathBuf {
        ...
        let nanos = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        path.push(format!("mfs_i18n_build_{nanos}"));
    ...
    fn build_rejects_unknown_formatter() {
        ...
        let catalog = Catalog {
            ...
            messages: vec![CatalogMessage {
                key: "home.title".to_string(),
                id: 1,
                args: vec![mfs_i18n_build::model::ArgSpec {
                    name: "value".to_string(),
                    arg_type: mfs_i18n_build::model::ArgType::String,
                    required: true,
                }],
                features: CatalogFeatures::default(),
                source_refs: None,
