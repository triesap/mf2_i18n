...
fn encode_message(program: &BytecodeProgram) -> Vec<u8> {
    ...
    for option in &program.formatter_options {
        ...
        match &option.value {
            mfs_i18n_core::FormatterOptionValue::Str(value) => {
                bytes.push(0);
                encode_inline_string(&mut bytes, value);
            }
            mfs_i18n_core::FormatterOptionValue::Num(value) => {
                bytes.push(1);
                bytes.extend_from_slice(&value.to_le_bytes());
            }
            mfs_i18n_core::FormatterOptionValue::Bool(value) => {
                bytes.push(2);
...
mod tests {
    ...
    use mfs_i18n_core::{BytecodeProgram, Catalog, MessageId, Opcode, PackCatalog, PackKind};
    use std::collections::BTreeMap;
